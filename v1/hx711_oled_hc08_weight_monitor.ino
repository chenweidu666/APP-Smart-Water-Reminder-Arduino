  ...
  if (digitalRead(HX711_DT) == LOW) {
    ...
    if (rawData != 0) {
      ...
      if (oledAvailable) {
        ...
        // 第三行：是否稳定
        static long lastDisplayWeight = 0;
        static unsigned long lastUpdateTime = 0;
        static bool isStable = false;
        if (millis() - lastUpdateTime > 200) { // 200ms刷新一次稳定性判断
          isStable = abs(weightGrams - lastDisplayWeight) <= 3;
          lastDisplayWeight = weightGrams;
          lastUpdateTime = millis();
        }
        display.print("Status: ");
        display.println(isStable ? "Stable" : "Unstable");
        display.display();
      }
